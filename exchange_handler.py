...
def get_symbol_leverage(session, symbol="POPCATUSDT"):
    ...
    Returns: leverage (float) or None if failed
    """
    try:
        # Get position info which includes leverage
...
def adjust_leverage(session, symbol, new_leverage):
    ...
    """
    try:
        # Check if we have cached leverage
        ...
        if response['retCode'] == 0:
            Leverage_amounts[symbol] = new_leverage  # Update cache
...
def get_position_info(session, symbol="POPCATUSDT"):
    ...
    Returns: Dictionary with:
        ...
        - size: Position size (float)
        - entry_price: Average entry price (float)
        - leverage: Current leverage (float)
        - pnl: Unrealized PnL (float)
    """
    try:
        position = session.get_positions(
            category="linear",
...
def handle_sell_signal(symbol, tp=None, sl=None, amount_usd=100):
    ...
    if not session:
        ...
        if not session:
            logger.info("Failed to initialize connection")
            return False
    symbol = symbol.split('.')[0]  # This removes everything after the dot.
